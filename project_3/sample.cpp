...
OsuSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
											...
											// ilng=NumLngs-1, lng=+M_PI are the same meridian
		{
			...
			if (Distort)
			{
				p->s = (lng + M_PI) / (2. * M_PI) + sin((lat + M_PI / 2. + Time)) ;
				p->t = (lat + M_PI / 2.) / M_PI + cos((lat + M_PI / 2. + Time)) ;
			}
			else
			{
				p->s = (lng + M_PI) / (2.*M_PI);
...
BmpToTexture(char* filename, int* width, int* height)
{
	...
	if (err != 0)
	{
		fprintf(stderr, "Cannot open Bmp file one '%s'\n", filename);
		return NULL;
	}
#else
	...
	if (fp == NULL)
	{
		fprintf(stderr, "Cannot open Bmp file second '%s'\n", filename);
		return NULL;
	}
#endif
