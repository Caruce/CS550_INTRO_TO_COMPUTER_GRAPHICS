...
const char* WINDOWTITLE = { "OpenGL / GLUT Sample -- Rui Gao" };
...
//	width BLADE_WIDTH centered at (0.,0.,0.) in the XY plane
	...
	glColor3f(1., 1., 1.);
	glBegin(GL_TRIANGLES);
	glVertex3f(BLADE_RADIUS, 2.9, (BLADE_WIDTH / 2. - 2.));
	glVertex3f(0., 2.9, -2.);
	glVertex3f(BLADE_RADIUS, 2.9, (-BLADE_WIDTH / 2. - 2.));
...
#define BLADE_RADIUS_R		3.0
#define BLADE_WIDTH_R		 1.0
...
//	width BLADE_WIDTH centered at (0.,0.,0.) in the XY plane
	...
	glColor3f(1., 1., 1.);
	glBegin(GL_TRIANGLES);
	glVertex3f(.5, BLADE_WIDTH_R / 2. + 2.5, 9. + BLADE_RADIUS_R);
	glVertex3f(.5, 2.5, 9.);
	glVertex3f(.5, -BLADE_WIDTH_R / 2. + 2.5, 9. + BLADE_RADIUS_R);
	...
	for (int i = 0; i < 512; i++)
	{
		glVertex3f(1.3 * cos(ang), 1.3 * sin(ang), -10.);
		glVertex3f(-1. * cos(ang), -1. * sin(ang), -10.);
		ang = ang + 2. * M_PI / (float)9.;
	}
...
MouseMotion(int x, int y)
{
	...
	if ((ActiveButton & LEFT) != 0 && WhichView != 1)
	{
		Xrot += (ANGFACT * dy);
		Yrot += (ANGFACT * dx);
	}

	if ((ActiveButton & MIDDLE) != 0 && WhichView != 1)
	{
